...
# iTerm tab and run claude". See README.md in this directory for the manual
# steps (Chrysalis, driver approval, macOS permissions) that this script
# cannot do for you.
#
...
#     other Karabiner rules, blindly copying would destroy them; merge the
#     "F18 -> new iTerm tab running claude" rule in by hand instead.
#
...
if [ -e "$DEST/karabiner.json" ] && [ "$FORCE" -eq 0 ]; then
  ...
  warn "Merge the 'F18 -> new iTerm tab running claude' rule into"
